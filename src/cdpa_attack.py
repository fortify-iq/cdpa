...
def cdpa_attack(data, traces, bit_count, share_count, verbose, list_traces):
    ...
    moments = [
        moment(traces[t], moment=share_count) if share_count > 1 else np.average(traces[t])
        for t in subsets
    ]
    ...
    for i in range(bit_count - 1):
        ...
        bit0 = int(abs(leaps[0]) > abs(leaps[1]))  # The position of the d2L maximal by abs value
    ...
    if verbose or list_traces:
        ...
        for j in range(2):
            ...
            if j >= 1:
                ...
                cprint(
                    '{:5.2f}'.format(dif), 'white', 'on_blue' if dif > 0 else 'on_yellow', end=''
                )
            for i in range(1, bit_count):
            ...
            for i in range(0, bit_count - 1):
                ...
                if abs(cur) > abs(leaps_trace[i][1 - j]):
                    cprint(
                        '{:5.2f}'.format(cur),
                        'white',
                        'on_blue' if cur < 0 else 'on_yellow',
                        end=''
                    )
                else:
    ...
    # The most significant bit of y contains the guessed XOR
    # of the most significant bits of x and y.
    return x, y
